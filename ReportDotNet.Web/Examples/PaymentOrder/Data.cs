...
namespace ReportDotNet.Web.Examples.PaymentOrder
{
    public class Data
    {
        ...
        private static string[] SplitInManyLines(string source,
                                                 params int[] linesLength)
        {
            source = source ?? "";
            var first = linesLength.Aggregate(new[] { source }, (current,
                                                                 l) =>
                                                                {
        ...
        private static string[] SplitIn2Lines(string source,
                                              int line1MaxLength)
        {
            source = source ?? "";
        ...
        {
            return EagerZip(first, second, (x,
                                            y) => new KeyValuePair<TFirst, TSecond>(x, y));
        }
